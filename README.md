# eqf-api-rest
fatbtc-api-rest
FatBTC Rest Api Example For Java：https://github.com/eqf/eqf-api-rest/tree/master/master/java/eqf-api-rest-java
Notices
...
3.The URL prefix in this example is: https://www.eqf.com (Recommended for users in mainland China access https://www.eqf.us)
...
7.get a list of assets getCurrencyList()
...
8.get the transaction details of a single order (only return the corresponding data of the current apikey) getSingleOrderDetail()
...
9.Get the order list (only return the corresponding data of the current apikey) getOrderList()
...
10.Get the transaction record (only return the corresponding data of the current apikey) getSuccessedOrders()
